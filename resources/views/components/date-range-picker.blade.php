...
<div id="{{ $pickerId }}_dateRangePickerContainer" class="relative inline-block w-full">
    <!-- Display Button -->
    <button 
        type="button" 
        id="{{ $btnId }}" 
    ...
    >
        <span id="{{ $displayId }}" class="truncate">Select dates</span>
    ...
    <input 
        ...
        name="check_in" 
        id="{{ $checkInInputId }}" 
        value="{{ $check_in ?? request('check_in', '') }}"
    >
    <input 
        type="hidden" 
        name="check_out" 
        id="{{ $checkOutInputId }}" 
        value="{{ $check_out ?? request('check_out', '') }}"
    >
</div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (window.__bhbsDatePickers && window.__bhbsDatePickers['{{ $pickerId }}']) return;
    window.__bhbsDatePickers = window.__bhbsDatePickers || {};
    window.__bhbsDatePickers['{{ $pickerId }}'] = true;
    
    const dateRangeBtn = document.getElementById('{{ $btnId }}');
    const dateRangePopup = document.getElementById('{{ $popupId }}');
    const dateRangeDisplay = document.getElementById('{{ $displayId }}');
    const checkInInput = document.getElementById('{{ $checkInInputId }}');
    const checkOutInput = document.getElementById('{{ $checkOutInputId }}');
    const clearDatesBtn = document.getElementById('{{ $pickerId }}_clearDatesBtn');
    const doneDatesBtn = document.getElementById('{{ $pickerId }}_doneDatesBtn');
    const flatpickrContainer = document.getElementById('{{ $containerId }}');

    if (!dateRangeBtn || !dateRangePopup || !dateRangeDisplay || !checkInInput || !checkOutInput || !flatpickrContainer) {
        console.warn('[DatePicker] Missing required elements for pickerId={{ $pickerId }}');
        return;
    }
