...
<section class="@if($sticky ?? false) sticky top-0 @endif bg-blue-600 text-white shadow-lg z-40">
    <div class="container mx-auto px-4 py-4">
        <!-- Search Form - Booking.com Style -->
        <div class="bg-white rounded-xl shadow-2xl p-3 max-w-6xl mx-auto">
            <form action="{{ route('guest.search') }}" method="GET" class="flex flex-wrap items-end gap-2 lg:gap-3">
                <!-- Dzongkhag -->
                <div class="flex-1 min-w-[160px]">
                    ...
                    <select name="dzongkhag_id" class="w-full px-3 py-2 lg:px-4 lg:py-3 border-2 border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 text-gray-900 bg-white h-10 lg:h-[50px] text-sm">
                        ...
                        @foreach($dzongkhags as $dzongkhag)
                            <option value="{{ $dzongkhag->id }}" {{ ($dzongkhag_id ?? request('dzongkhag_id')) == $dzongkhag->id ? 'selected' : '' }}>{{ $dzongkhag->name }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div class="flex-1 min-w-[160px] relative">
                    ...
                    <div id="guestPickerPopup" class="hidden absolute top-full right-0 bg-white rounded-lg shadow-2xl border border-gray-200 z-50 w-80 p-4 lg:p-6 mt-1">
                        ...
                        <div class="mb-4 lg:mb-6">
                            ...
                            <div class="flex items-center justify-between bg-gray-50 rounded-lg p-2 lg:p-3">
                                ...
                                <input type="number" name="adults" id="adultsInput" min="1" value="{{ $adults ?? request('adults', 1) }}" class="guest-input w-14 text-center text-base lg:text-lg font-bold border-0 bg-transparent" readonly>
                        ...
                        <div class="mb-4 lg:mb-6">
                            ...
                            <div class="flex items-center justify-between bg-gray-50 rounded-lg p-2 lg:p-3">
                                ...
                                <input type="number" name="children" id="childrenInput" min="0" value="{{ $children ?? request('children', 0) }}" class="guest-input w-14 text-center text-base lg:text-lg font-bold border-0 bg-transparent" readonly>
                        ...
                        <div class="mb-4 lg:mb-6">
                            ...
                            <div class="flex items-center justify-between bg-gray-50 rounded-lg p-2 lg:p-3">
                                ...
                                <input type="number" name="rooms" id="roomsInput" min="1" value="{{ $rooms ?? request('rooms', 1) }}" class="guest-input w-14 text-center text-base lg:text-lg font-bold border-0 bg-transparent" readonly>
                    ...
                    <input type="hidden" name="adults" value="{{ $adults ?? request('adults', 1) }}">
                    <input type="hidden" name="children" value="{{ $children ?? request('children', 0) }}">
                    <input type="hidden" name="rooms" value="{{ $rooms ?? request('rooms', 1) }}">
                </div>
