...
<head>
    ...
    <title>Edit Guest - Reception Dashboard</title>
        ...
        <div class="flex-1 overflow-x-hidden">
            ...
            <header class="bg-white shadow-sm border-b">
                <div class="px-8 py-4 flex justify-between items-center">
                    <div>
                        <h2 class="text-2xl font-bold text-gray-800">Edit Guest</h2>
                        <p class="text-gray-600 text-sm">Update guest information</p>
                    </div>
            ...
            <main class="p-8">
                <div class="max-w-2xl mx-auto">
                    <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-8">
                        <form method="POST" action="{{ route('reception.guests.update', $guest->id) }}">
                            @csrf
                            @method('PUT')
                            ...
                            <div class="mb-6">
                                ...
                                <textarea name="address" rows="3"
                                    class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500">{{ old('address', $guest->address) }}</textarea>
                                @error('address')
                                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                                @enderror
                            </div>
