...
<div class="dashboard-card">
    <div class="card-body">
        <form action="{{ route('admin.hotels.update', $hotel->id) }}" method="POST" class="admin-form">
            @csrf
            @method('PUT')
                ...
                <div class="form-group">
                    ...
                    <select name="owner_id" id="owner_id" class="form-control" required>
                        ...
                        @foreach($users as $user)
                            <option value="{{ $user->id }}" {{ (old('owner_id') ?? $hotel->owner_id) == $user->id ? 'selected' : '' }}>
            ...
            <div class="form-row">
                <div class="form-group">
                    ...
                    <select name="property_type" id="property_type" class="form-control" required>
                        ...
                        <option value="HOTEL" {{ (old('property_type') ?? $hotel->property_type) == 'HOTEL' ? 'selected' : '' }}>Hotel</option>
                        <option value="GUEST_HOUSE" {{ (old('property_type') ?? $hotel->property_type) == 'GUEST_HOUSE' ? 'selected' : '' }}>Guest House</option>
                        <option value="RESORT" {{ (old('property_type') ?? $hotel->property_type) == 'RESORT' ? 'selected' : '' }}>Resort</option>
                        <option value="HOMESTAY" {{ (old('property_type') ?? $hotel->property_type) == 'HOMESTAY' ? 'selected' : '' }}>Homestay</option>
                    </select>
                    @error('property_type')
                        <span class="error-message">{{ $message }}</span>
                    @enderror
                ...
                <!-- Dzongkhag field removed - no longer in schema -->
            </div>

            <div class="form-row">
                <div class="form-group">
                    <label for="phone">Phone <span class="required">*</span></label>
                    <input type="text" name="phone" id="phone" class="form-control" value="{{ old('phone') ?? $hotel->phone }}" required>
                    @error('phone')
                        <span class="error-message">{{ $message }}</span>
                    @enderror
                </div>
            ...
            <div class="form-row">
                ...
                <div class="form-group">
                    ...
                    <select name="status" id="status" class="form-control" required>
                        ...
                        <option value="APPROVED" {{ (old('status') ?? $hotel->status) == 'APPROVED' ? 'selected' : '' }}>Approved</option>
            ...
            <div class="form-group">
                ...
                <textarea name="address" id="address" class="form-control" rows="2" required>{{ old('address') ?? $hotel->address }}</textarea>
                @error('address')
                    <span class="error-message">{{ $message }}</span>
                @enderror
            </div>
