...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-8 mb-8">
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-8 text-center">
        ...
        <h3 class="text-xl font-bold text-gray-900 mb-2">{{ $guest->guest_name }}</h3>
        <p class="text-gray-600 mb-1">{{ $guest->email }}</p>
        <p class="text-gray-600">{{ $guest->mobile ?? 'No mobile' }}</p>
    </div>
    ...
    <div class="lg:col-span-2 grid grid-cols-1 md:grid-cols-3 gap-6">
        <div class="bg-gradient-to-br from-blue-500 to-blue-600 rounded-xl shadow-lg p-6 text-white">
            ...
            <p class="text-2xl font-bold">{{ $guest->total_bookings }}</p>
            <p class="text-sm opacity-90">Total Bookings</p>
        </div>
        <div class="bg-gradient-to-br from-green-500 to-green-600 rounded-xl shadow-lg p-6 text-white">
            <i class="fas fa-chart-line text-3xl mb-3 opacity-80"></i>
            <p class="text-2xl font-bold">Nu. {{ number_format($guest->total_spent ?? 0, 2) }}</p>
        ...
        <div class="bg-gradient-to-br from-purple-500 to-purple-600 rounded-xl shadow-lg p-6 text-white">
            ...
            <p class="font-semibold">{{ \Carbon\Carbon::parse($guest->last_visit)->format('M d, Y') }}</p>
        </div>
    </div>
</div>
